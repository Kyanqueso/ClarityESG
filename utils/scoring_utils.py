...
def clean_sme_name(name):
    replace_words = ["INCORPORATED", "CORPORATION",
                     "RESPONDENT", "INC", "COMPANY", ".", ","]
    name = name.upper()
...
def sector_risk_avg(sector_name):
    conn = sqlite3.connect("esg_scoring.db")
    df = pd.read_sql(
        "SELECT * FROM esg_sector_risks WHERE sector = ?", conn, params=(sector_name,))
...
def region_risk(region_name):
    conn = sqlite3.connect("esg_scoring.db")
    df = pd.read_sql("SELECT * FROM region_risks WHERE region = ?",
...
def score_sme(sme_id, industry_sector, region, db_path="esg_scoring.db"):
    ...
    sme = pd.read_sql("""SELECT * FROM sme where sme_id=?""",
                      conn, params=(sme_id,))
    if sme.empty:
        conn.close()
    ...
    financial_components['sector_stability'] = abs(
        normalize(float(sector_score['avg_score'].iloc[0]), 0, 10) - 100)
    financial_components['market_competition'] = abs(
        # need source
        normalize(int(sme['market_competition'].iloc[0]), 0, 10) - 100)

    financial_score = sum(financial_components.values()
                          ) / len(financial_components)
    ...
    if pd.isna(raw_value2):
        ...
        water_use = water_use = float(
            str(raw_value2).replace("m3", "").replace("l", "").strip())
    ...
    for supplier_name, supplier_sector, supplier_region, supplier_permit in zip(data.supplier_name,
        ...
        sector_score = abs(
            normalize(float(sector_df["avg_score"].iloc[0]), 0, 10)-100)
        ...
        final_score = ((name_score + sector_score +
                       region_score + permit_score) / 4)
        supplier_scores.append(final_score)
        print(final_score)

    average_supplier_score = np.mean(
        supplier_scores) if supplier_scores else 100

    # Calculate grand total score
    base_score = (0.25 * financial_score + 0.25 * env_score_bonus +
                  0.25 * soc_score + 0.25 * gov_score_bonus)
